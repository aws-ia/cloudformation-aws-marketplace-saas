...
1. Ensure that you are deploying in a supported region (see link:#_supported_regions[Supported Regions])
2. Ensure that you are deploying into your registered seller account.
3. Ensure that the names assigned for WebsiteS3BucketName, NewSubscribersTableName, and AWSMarketplaceMeteringRecordsTableName don't already exist within S3 and DynamoDB.